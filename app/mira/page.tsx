...
function loadState(): MiraState {
  ...
  try {
    ...
    const memories = Array.isArray(game?.memories) ? game.memories.map((text: string) => ({ text, createdAt: Date.now() })) : [];
    return { ...defaultState, ...(saved || {}), memories: saved?.memories?.length ? saved.memories : memories };
  } catch { return defaultState; }
}

function inferFromGame(): MiraState {
  if (typeof window === 'undefined') return defaultState;
  try {
    const game = JSON.parse(localStorage.getItem(SAVE) || 'null');
    if (!game) return defaultState;
    return {
      ...defaultState,
...
export default function MiraPage() {
  ...
  const [messages, setMessages] = useState<{ role: 'mira' | 'user'; text: string }[]>([]);
  ...
  function persist(next: MiraState) {
    setState(next);
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      const res = await fetch('/api/mira', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ message: text, state }) });
      const data = await res.json();
      const reply = data.reply || 'Я рядом. Расскажи ещё.';
      setMessages(m => [...m, { role: 'mira', text: reply }]);
      const next: MiraState = { ...state, closeness: state.closeness + 1, memories: [...state.memories, { text: `Игрок сказал: ${text}`, createdAt: Date.now() }].slice(-30) };
      persist(next);
    } catch {
  ...
  return (
    <main className="mira-shell">
      <section className="mira-card">
        <header className="mira-head">
          <div><span className="eyebrow">BETWEEN US / MIRA</span><h1>Мира</h1><p>Она помнит. И отношения с ней меняются.</p></div>
          <div className="pulse" aria-label={`Близость ${relationship}%`}><span>{relationship}</span><small>близость</small></div>
        </header>

        <div className="memory-strip">{state.memories.length ? `Память: ${state.memories.length} воспоминаний` : 'Память пока пуста'} · доверие {state.trust} · напряжение {state.tension}</div>

        <div className="chat" aria-live="polite">
          {messages.map((m, i) => <div key={i} className={`bubble ${m.role}`}>{m.text}</div>)}
          {busy && <div className="bubble mira typing">Мира печатает…</div>}
        </div>

        <div className="composer">
          <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder="Напиши Мире…" rows={2} />
          <button onClick={send} disabled={busy || !input.trim()}>Отправить</button>
        </div>
